// 合并器：把第一批（87 条）与第二批（119 条）裁定合并为单一可审计文件。
//
// 合并规则：
//   1. 以"去空白与标点后的正文"为去重键；两批出现同一正文时保留先裁定的第一批结果，
//      第二批同正文记录标记为 duplicate_of，不重复计数。
...
// 合并器只读 batch1 + batch2、只写合并结果 —— 这样重复运行是幂等的，
...
for (const r of [...b1Records, ...b2.records]) {
...
const out = {
  ...
  queueId: `merged:${b1.queueId}+${b2.queueId}`,
  adjudicatedAt: new Date().toISOString(),
  adjudicatorNote: '第一批（87 条，含 14 条待复核的再裁定）与第二批（119 条）合并结果。跨批次同正文已去重，保留先裁定结果。attributionBasis 区分文本直证与父内容放宽，可分别统计。',
  batches: [
