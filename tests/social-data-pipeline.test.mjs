...
test('social research pipeline: deduplicates, redacts and never creates a reputation score', () => {
  ...
  try {
    ...
    assert.equal(summary.totals.uniqueRecords, 5);
    assert.equal(summary.totals.quarantineRecords, 1);
    assert.equal(summary.sources[0].duplicateRows, 1);
    assert.equal(summary.flagCounts.direct_identifier_redacted, 1);
    assert.equal(summary.flagCounts.high_risk_personal_disclosure, 1);
    ...
    assert.doesNotMatch(privateText, /13800138000|\"note_id\"|\"comment_id\"|sourceKeyword|source_keyword|nickname|user_id/);
