export const DATA_COLLECTIONS = ['drugs', 'disorders', 'cases', 'resources'];
...
const PRIVATE_DIRECTORY_LABELS = ['raw', 'work', 'tmp']
  .map((name) => `${name}${String.fromCharCode(47)}`)
  .join('、');
const FILE_URL_LABEL = ['file:', '/', '/'].join('');
...
function validateResourceUrl(errors, item, id) {
  ...
  if (PRIVATE_DIRECTORY_PATTERN.test(url)) {
    addError(errors, 'resources', id, 'url', `不得指向 ${PRIVATE_DIRECTORY_LABELS} 私有目录`);
    return;
  }
  if (FILE_URL_PATTERN.test(url)) {
    addError(errors, 'resources', id, 'url', `不得使用 ${FILE_URL_LABEL} URL`);
    return;
  }
    ...
    collections[type].forEach((item, index) => {
      ...
      visitStrings(item, '', (value, field) => {
        ...
        if (LOCAL_ABSOLUTE_PATH_PATTERN.test(value) || FILE_URL_PATTERN.test(value)) {
          addError(errors, type, id, field, `包含本地绝对路径、UNC 网络路径或 ${FILE_URL_LABEL} URL`);
        }
      });
    });
  });
