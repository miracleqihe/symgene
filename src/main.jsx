...
function DrugDetailSections({ item }) {
  ...
  function selectFromKeyboard(event, index) {
    ...
    event.preventDefault();
    const nextSection = DRUG_DETAIL_SECTIONS[nextIndex];
    setActiveSection(nextSection.id);
    tabRefs.current[nextIndex]?.focus();
  }

  return <div className="drug-detail-sections"><div className="drug-section-layout"><div className="drug-section-tabs" role="tablist" aria-label={`${item.name}内容目录`}>{DRUG_DETAIL_SECTIONS.map((section, index) => { const selected = activeSection === section.id; const tabId = `drug-tab-${item.id}-${section.id}`; const panelId = `drug-panel-${item.id}-${section.id}`; return <button type="button" role="tab" id={tabId} aria-controls={panelId} aria-selected={selected} tabIndex={selected ? 0 : -1} ref={(node) => { tabRefs.current[index] = node; }} className="drug-section-tab" style={{ '--section-color': section.color, '--section-foreground': section.foreground }} onClick={() => setActiveSection(section.id)} onKeyDown={(event) => selectFromKeyboard(event, index)} key={section.id}><span className="drug-section-tab-index" aria-hidden="true">{String(index + 1).padStart(2, '0')}</span><span className="drug-section-tab-label">{section.label}</span></button>; })}</div><div className="drug-section-content">{DRUG_DETAIL_SECTIONS.map((section) => { const selected = activeSection === section.id; return <section role="tabpanel" id={`drug-panel-${item.id}-${section.id}`} aria-labelledby={`drug-tab-${item.id}-${section.id}`} hidden={!selected} className={'drug-section-panel ' + (selected ? 'is-active' : '')} style={{ '--section-color': section.color }} key={section.id}><span className="drug-section-kicker">药物手册 · {section.label}</span><h3>{section.label}</h3><p>{item[section.id] || '待补充'}</p></section>; })}</div></div><Fact label="禁忌与警示" text={item.contraindications} warning className="drug-warning" /></div>;
}
...
function Fact({ label, text, warning, priority, className = '' }) { return <div className={'fact ' + (warning ? 'warning ' : '') + (priority ? 'fact-primary ' : 'fact-secondary ') + className}><span>{warning && <ShieldCheck size={14} />}{label}</span><p>{text || '待补充'}</p></div>; }
